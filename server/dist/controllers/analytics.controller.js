...
exports.getCrossPlatformInsights = exports.getYouTubeHistoricalData = exports.analyzeYouTubeComments = exports.getYouTubeStats = exports.getYouTubeAnalysis = void 0;
 ...
 */
const getYouTubeAnalysis = async (req, res) => {
    try {
        ...
        try {
            ...
            if (channelResponse.data.items && channelResponse.data.items.length > 0) {
                ...
                const videosResponse = await axios_1.default.get('https://www.googleapis.com/youtube/v3/playlistItems', {
                    params: {
                ...
                if (videosResponse.data.items) {
                    ...
                    const videoStatsResponse = await axios_1.default.get('https://www.googleapis.com/youtube/v3/videos', {
                        params: {
                    ...
                    youtubeData.recentVideos = videosResponse.data.items.map((video, index) => {
        ...
        const aiAnalysis = await ai_analysis_service_1.aiAnalysisService.analyzeYouTubeData(youtubeData);
        res.json({
            success: true,
            data: youtubeData,
            analysis: aiAnalysis,
...
exports.getYouTubeAnalysis = getYouTubeAnalysis;
/**
...
exports.getYouTubeStats = getYouTubeStats;
/**
 * Analyze YouTube comments for sentiment and themes
 */
const analyzeYouTubeComments = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
        ...
        try {
            ...
            for (const videoId of videoIds) {
                try {
                    const commentsResponse = await axios_1.default.get('https://www.googleapis.com/youtube/v3/commentThreads', {
                        params: {
                            part: 'snippet',
                            videoId: videoId,
                            maxResults: 20,
            ...
            const analysis = await ai_analysis_service_1.aiAnalysisService.analyzeComments(commentsToAnalyze);
            res.json({
                success: true,
                hasData: true,
                analysis,
...
exports.analyzeYouTubeComments = analyzeYouTubeComments;
/**
 * Get YouTube historical analytics
 * Returns REAL view data from YouTube Analytics API
 */
const getYouTubeHistoricalData = async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        try {
            ...
            try {
                ...
                do {
                    const playlistResponse = await axios_1.default.get('https://www.googleapis.com/youtube/v3/playlistItems', {
                        params: {
                            part: 'contentDetails',
                            playlistId: uploadsPlaylistId,
                            maxResults: 50,
...
exports.getYouTubeHistoricalData = getYouTubeHistoricalData;
/**
 * Get cross-platform insights
 */
const getCrossPlatformInsights = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
        ...
        const insights = await ai_analysis_service_1.aiAnalysisService.generateQuickInsights(platformData);
        res.json({
            success: true,
...
exports.getCrossPlatformInsights = getCrossPlatformInsights;
