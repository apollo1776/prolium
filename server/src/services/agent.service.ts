...
export class AgentService {
  ...
  async processMessage(
    ...
    // Build system prompt
    const platformList = connectedPlatforms.map(p => `${p.platform}${p.username ? ` (@${p.username})` : ''}`).join(', ') || 'none connected yet';
...
- Use emojis sparingly to indicate actions (✅ done, 🔍 researching, 📝 creating, 📱 ready to post)
