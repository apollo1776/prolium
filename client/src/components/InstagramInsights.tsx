...
import { Sparkles, Camera, Loader2, TrendingUp, Users, Eye, MessageCircle, Heart } from 'lucide-react';
...
const InstagramInsights: React.FC = () => {
  ...
  const loadStats = async () => {
    ...
    try {
      const response = await apiClient.get('/analytics/instagram/stats');
      setStats(response.data);
    } catch (error: any) {
      console.error('Stats error:', error);
      setStatsError(
        error.response?.data?.message || 'Connect your Instagram account to view statistics'
      );
    } finally {
  ...
  const generateAnalysis = async () => {
    ...
    try {
      const response = await apiClient.get('/analytics/instagram');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-white">Instagram AI Insights</h2>
          <p className="text-gray-500">AI-powered analysis of your Instagram account</p>
        </div>
        <button
      ...
      {statsLoading && (
        <div className="bg-black border border-[#1a1a1a] rounded-xl p-8 text-center">
          ...
          <p className="text-gray-400 text-sm">Loading Instagram statistics...</p>
        </div>
      )}
      ...
      {statsError && (
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
          ...
          <p className="text-gray-400 text-xs mt-2">
            Go to Settings to connect your Instagram account
          </p>
        </div>
      )}
      ...
      {stats?.hasAccount && !statsLoading && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-black border border-[#1a1a1a] rounded-xl p-6">
            ...
            <p className="text-2xl font-bold text-white">
              {stats.stats.mediaCount?.toLocaleString() || '0'}
            </p>
          </div>
          ...
          <div className="bg-black border border-[#1a1a1a] rounded-xl p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-gray-400 text-sm">Avg. Likes</span>
            </div>
            <p className="text-2xl font-bold text-white">
              {stats.stats.avgLikes?.toLocaleString() || '0'}
            </p>
          </div>
          ...
          <div className="bg-black border border-[#1a1a1a] rounded-xl p-6">
            ...
            <p className="text-2xl font-bold text-white">
              {stats.stats.engagementRate ? `${stats.stats.engagementRate}%` : '0%'}
            </p>
          </div>
        </div>
      )}
      ...
      {loading && !analysis && (
        <div className="bg-black border border-[#1a1a1a] rounded-xl p-12 text-center">
          ...
          <p className="text-gray-300 font-medium">Analyzing your Instagram data...</p>
            ...
            <div className="bg-black border border-[#1a1a1a] rounded-xl p-6">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-white font-medium capitalize">{analysis.data.account.accountType || 'Personal'}</p>
                </div>
              </div>
            </div>
          )}
      ...
      {!analysis && !loading && !error && stats?.hasAccount && !statsLoading && (
        <div className="bg-[#1a1a1a]/30 border border-slate-700 rounded-xl p-12 text-center">
          ...
          <p className="text-gray-400 mb-6">
            Click "Generate AI Analysis" above to get personalized recommendations and deep insights about your Instagram account
          </p>
        </div>
      )}
    </div>
