...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const checkAuth = async () => {
          try {
            ...
            // Read existing session BEFORE calling server - preserve the user's rememberMe preference
            ...
            // Preserve the user's rememberMe preference from localStorage if it exists
            // Only fall back to backend-inferred value if no local session exists
            ...
            saveSession(response.data.user.id, response.data.user.email, rememberMe);
                  console.log('[AuthContext] Session restored, rememberMe:', rememberMe, '(source:', existingSession !== null ? 'localStorage' : 'backend', ')');
          } catch (error: any) {
    ...
    const login = async (email: string, password: string, rememberMe = false) => {
          ...
          // Save session to localStorage with the user's explicit rememberMe choice
    ...
    const refreshUser = async () => {
          try {
                  // Preserve rememberMe preference before refreshing
            const existingSession = getSession();
            ...
            if (response.data.user) {
                      // Use existing localStorage rememberMe preference, not backend-inferred value
                    const rememberMe = existingSession !== null
                        ? existingSession.rememberMe
                                : (response.data.user.rememberMe ?? false);
                      saveSession(response.data.user.id, response.data.user.email, rememberMe);
